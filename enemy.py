...
class Enemy(Entity):
    ...
    def get_status(self, player):
        distance = self.player_direction(player)[0]
    ...
    def cooldown(self):
        ...
        if not self.can_attack:
            if not current_time - self.attack_time >= self.attack_cd:
                self.can_attack = True

        if not self.vunerable:
            if current_time - self.hit_time >= self.invecibility_cd:
                self.vunerable = True
